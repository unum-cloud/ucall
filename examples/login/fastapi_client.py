...
class ClientREST:
    ...
    def __init__(
        self, uri: str = "127.0.0.1", port: int = 8000, identity: int = PROCESS_ID
    ) -> None:
    ...
    def __call__(self, *, a: Optional[int] = None, b: Optional[int] = None) -> int:
        ...
        result = requests.get(
            f"{self.url}validate_session?user_id={a}&session_id={b}"
        ).text
        c = int(result)
...
class ClientRESTReddit:
    ...
    def __init__(
        ...
        self.bio = "".join(random.choices(string.ascii_uppercase, k=self.bin_len))
        self.name = "John"

    def __call__(self) -> int:
        age = random.randint(1, 1000)
        result = requests.get(
            f"{self.url}create_user?age={age}&bio={self.bio}&name={self.name}&text={self.bio}"
        ).text
        return result
