...
struct key_equal {
    ...
    bool operator()(key_value_pair const& pair, std::string_view key) const noexcept { return pair.key() == key; }
    bool operator()(std::string_view key, key_value_pair const& pair) const noexcept { return key == pair.key(); }
};

static std::shared_mutex store_mutex;
static std::unordered_set<key_value_pair> store;
