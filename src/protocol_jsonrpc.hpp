...
template <typename base_protocol_t> struct protocol_jsonrpc_t {
    base_protocol_t base_protocol{};
    jsonrpc_object_t active_request{};
    sjd::parser parser{};
...
inline void protocol_jsonrpc_t<base_protocol_t>::prepare_response(exchange_pipes_t& pipes) noexcept {
...
template <typename base_protocol_t>
inline bool protocol_jsonrpc_t<base_protocol_t>::append_response(exchange_pipes_t& pipes,
                                                                 std::string_view response) noexcept {
    // Communication example would be:
    // --> {"jsonrpc": "2.0", "method": "subtract", "params": [42, 23], "id": 1}
    // <-- {"jsonrpc": "2.0", "id": 1, "result": 19}
    if (active_request.dynamic_id.empty())
        return true;
    if (!pipes.append_outputs({R"({"jsonrpc":"2.0","id":)", 22}))
        return false;
    if (!pipes.append_outputs(active_request.dynamic_id))
        return false;
...
template <typename base_protocol_t>
inline bool protocol_jsonrpc_t<base_protocol_t>::append_error(exchange_pipes_t& pipes, std::string_view error_code,
...
template <typename base_protocol_t>
inline void protocol_jsonrpc_t<base_protocol_t>::finalize_response(exchange_pipes_t& pipes) noexcept {
    // Drop last comma.
...
template <typename base_protocol_t>
bool protocol_jsonrpc_t<base_protocol_t>::is_input_complete(span_gt<char> input) noexcept {
    return base_protocol.is_input_complete(input);
}

template <typename base_protocol_t> void protocol_jsonrpc_t<base_protocol_t>::reset() noexcept {
    base_protocol.reset();
}

template <typename base_protocol_t>
inline std::optional<default_error_t>
protocol_jsonrpc_t<base_protocol_t>::parse_headers(std::string_view body) noexcept {
    return base_protocol.parse_headers(body);
}

template <typename base_protocol_t>
inline std::optional<default_error_t> protocol_jsonrpc_t<base_protocol_t>::parse_content() noexcept {
...
template <typename base_protocol_t>
inline any_param_t
protocol_jsonrpc_t<base_protocol_t>::as_variant(sj::simdjson_result<sjd::element> const& elm) const noexcept {
    if (elm.is_bool())
...
template <typename base_protocol_t>
inline std::string_view protocol_jsonrpc_t<base_protocol_t>::get_content() const noexcept {
    return base_protocol.get_content();
}

template <typename base_protocol_t>
inline request_type_t protocol_jsonrpc_t<base_protocol_t>::get_request_type() const noexcept {
    return base_protocol.get_request_type();
}

template <typename base_protocol_t>
inline any_param_t protocol_jsonrpc_t<base_protocol_t>::get_param(std::string_view name) const noexcept {
    ...
    return as_variant(active_request.element.at_pointer({json_pointer, final_size}));
}

template <typename base_protocol_t>
inline any_param_t protocol_jsonrpc_t<base_protocol_t>::get_param(std::size_t position) const noexcept {
    char json_pointer[json_pointer_capacity_k]{};
    std::memcpy((void*)json_pointer, "/params/", 8u);
...
template <typename base_protocol_t>
inline std::string_view protocol_jsonrpc_t<base_protocol_t>::get_header(std::string_view header_name) const noexcept {
    return base_protocol.get_header(header_name);
}

template <typename base_protocol_t>
std::optional<default_error_t> protocol_jsonrpc_t<base_protocol_t>::set_to(sjd::element const& doc) noexcept {
    if (!doc.is_object())
    ...
    if (id.is_string()) {
        active_request.dynamic_id = id.get_string().value_unsafe();
    } else if (id.is_int64() || id.is_uint64()) {
        char* code = &active_request.printed_int_id[0];
        ...
        active_request.dynamic_id = std::string_view(code, code_len);
    } else
        active_request.dynamic_id = "";

    active_request.method_name = method.get_string().value_unsafe();
    active_request.element = doc;
    return std::nullopt;
}
